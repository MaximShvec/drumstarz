  ...
  return (
    <footer className="border-t border-white/8 bg-void-2">
      <div className="container-site grid gap-12 py-16 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-cream/55">
            Школа барабанов в центре Риги с 2013 года. Индивидуальные занятия, перкуссия, мероприятия и летний лагерь.
          </p>
        ...
        <FooterCol title="Школа барабанов">
          {FOOTER_SCHOOL.map((item) => (
            <li key={item.label}>
              <Link to={item.to} className="link-draw text-sm text-cream/60">
                {item.label}
              </Link>
            </li>
          ))}
          <li>
            <span className="text-sm text-cream/30">Новости</span>
          </li>
        </FooterCol>

        <FooterCol title="Другие форматы">
        ...
        <FooterCol title="Контактная информация">
          <li>
      ...
      <div className="container-site flex flex-col gap-3 border-t border-white/8 py-6 text-xs text-cream/35 sm:flex-row sm:items-center sm:justify-between">
        <p>DRUMSTARZ © {SITE.year}. Все права защищены.</p>
