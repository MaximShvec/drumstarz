        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <Reveal>
            <div className="rounded-[1.6rem] bg-void p-8 text-cream lg:sticky lg:top-28">
              <h3 className="font-display text-2xl font-semibold leading-tight">{copy.faq.cardTitle}</h3>
              <p className="mt-3 text-sm text-cream/60">{copy.faq.cardLead}</p>
              <div className="mt-8 space-y-5">
                <LineInput placeholder={t.booking.name} aria-label={t.booking.name} autoComplete="name" />
                <LineInput placeholder={t.booking.phone} aria-label={t.booking.phone} autoComplete="tel" type="tel" />
                <Button className="w-full py-4" onClick={openBooking}>
                  {t.booking.submit}
                </Button>
                <p className="text-center text-xs uppercase tracking-[0.16em] text-cream/35">{t.booking.reply}</p>
              </div>
            </div>
          </Reveal>
        </div>
